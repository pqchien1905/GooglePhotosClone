...
import ShareAlbumsEmailPreview from '@/Components/ShareAlbumsEmailPreview';
...
interface Props {
  ...
  onClose: () => void;
  albumIds: number[];
  albums?: Album[]; // Optional: pass albums data for preview
  senderName?: string; // Optional: sender name for preview
}

type Tab = 'link' | 'friends' | 'email';

export default function ShareAlbumsModal({ isOpen, onClose, albumIds = [], albums = [], senderName }: Props) {
  ...
  const handleCreateLink = async () => {
    ...
    if (albumIds.length > 1) {
      setLinkError('Chỉ có thể tạo link cho 1 album. Vui lòng chọn 1 album để tạo link.');
      return;
    }
    ...
    setLinkError(null);
    try {
      const payload: any = { type: 'album', id: albumIds[0] };
  ...
  const handleShareToFriends = () => {
    ...
    if (albumIds.length > 0) payload.album_id = albumIds[0];
  ...
  const handleShareViaEmail = async () => {
    ...
    try {
      await axios.post(route('albums.share-email'), {
        album_ids: albumIds,
        emails: emails,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="w-full max-w-2xl rounded-xl bg-white shadow-xl dark:bg-gray-800">
            ...
            <div className="space-y-4">
              {albumIds.length > 1 && (
                <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-3 text-sm text-yellow-800 dark:border-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300">
                  Chỉ có thể tạo link cho 1 album. Vui lòng chọn 1 album để tạo link.
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Gửi lời nhắn kèm theo album..."
                />
              </div>

              {albumIds.length > 0 && !showEmailPreview && (
                <Button
                  variant="outline"
                  size="sm"
              ...
              {showEmailPreview && (
                <div className="space-y-4 border-t pt-4 dark:border-gray-700">
                  ...
                  <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden max-h-[500px] overflow-y-auto bg-white">
                    <ShareAlbumsEmailPreview
                      senderName={senderName || 'Người dùng'}
                      albums={albums.length > 0 
                        ? albums.filter(a => albumIds.includes(a.id))
                        : albumIds.map(id => ({ id, name: 'Album', photos_count: 0 }))
                      }
        ...
        <div className="flex items-center justify-end gap-2 border-t px-6 py-4 dark:border-gray-700">
          ...
          {activeTab === 'link' && albumIds.length === 1 && !shareUrl && (
