...
import ShareVideosEmailPreview from '@/Components/ShareVideosEmailPreview';
...
interface Props {
  ...
  onClose: () => void;
  videoIds: number[];
  videos?: Video[]; // Optional: pass videos data for preview
  senderName?: string; // Optional: sender name for preview
}

type Tab = 'link' | 'friends' | 'email';

export default function ShareVideosModal({ isOpen, onClose, videoIds = [], videos = [], senderName }: Props) {
  ...
  const handleCreateLink = async () => {
    ...
    if (videoIds.length > 1) {
      setLinkError('Chỉ có thể tạo link cho 1 video. Vui lòng chọn 1 video để tạo link.');
      return;
    }
    ...
    setLinkError(null);
    try {
      const payload: any = { type: 'photo', id: videoIds[0] };
  ...
  const handleShareToFriends = () => {
    ...
    if (videoIds.length > 0) payload.photo_ids = videoIds;
  ...
  const handleShareViaEmail = async () => {
    ...
    try {
      await axios.post(route('videos.share-email'), {
        video_ids: videoIds,
        emails: emails,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="w-full max-w-2xl rounded-xl bg-white shadow-xl dark:bg-gray-800">
            ...
            <div className="space-y-4">
              {videoIds.length > 1 && (
                <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-3 text-sm text-yellow-800 dark:border-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300">
                  Chỉ có thể tạo link cho 1 video. Vui lòng chọn 1 video để tạo link.
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Gửi lời nhắn kèm theo video..."
                />
              </div>

              {videoIds.length > 0 && !showEmailPreview && (
                <Button
                  variant="outline"
                  size="sm"
        ...
        <div className="flex items-center justify-end gap-2 border-t px-6 py-4 dark:border-gray-700">
          ...
          {activeTab === 'link' && videoIds.length === 1 && !shareUrl && (
