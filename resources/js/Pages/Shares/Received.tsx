...
import { Share2, Image as ImageIcon, FolderOpen, User, Clock, MoreVertical, Download, Play, Cloud } from 'lucide-react';
...
export default function SharesReceived({ shares }: Props) {
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {shares.data.length} mục được chia sẻ với bạn
            </p>
          </div>
          ...
          {shares.data.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24">
              <div className="text-center max-w-md">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Các ảnh và album được chia sẻ với bạn sẽ hiển thị ở đây
                </p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-2 gap-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7">
                {shares.data.map((share) => (
                  ...
                  >
                    ...
                    {/* Unread Indicator */}
                    {!share.is_read && (
                      <div className="absolute top-2 right-2 w-2.5 h-2.5 bg-blue-500 rounded-full border-2 border-white dark:border-gray-900"></div>
                    )}
                  </div>
                ))}
              </div>
              ...
              {shares.last_page > shares.current_page && (
                <div className="flex justify-center mt-12">
                  <button
                    onClick={() => router.get(route('shares.received'), { page: shares.current_page + 1 }, { preserveState: true, preserveScroll: true })}
